...
const StrategicPillarsSection = () => {
  ...
  return <section className="relative py-20 bg-gradient-to-b from-background to-card overflow-hidden">
      <div className="container mx-auto px-4 relative z-10">
        ...
        {/* Headline principal */}
        <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-center mb-16 animate-fade-in" style={{
        animationDelay: '0.1s'
      }}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 md:gap-6 max-w-6xl mx-auto mb-8 animate-fade-in" style={{
        animationDelay: '0.2s'
      }}>
          {pillars.map((pillar, index) => {
          const Icon = pillar.icon;
          return <div key={index} className="group relative bg-card border border-border/50 rounded-xl p-6 text-center hover:border-primary/50 hover:shadow-lg hover:shadow-primary/10 transition-all duration-300">
                {/* Ícone */}
        ...
        <div className="relative h-24 md:h-32 max-w-6xl mx-auto mb-8 animate-fade-in" style={{
        animationDelay: '0.3s'
      }}>
          <svg className="w-full h-full" viewBox="0 0 1200 120" preserveAspectRatio="xMidYMid meet" fill="none" xmlns="http://www.w3.org/2000/svg">
            {/* Linhas curvas partindo de cada card convergindo ao centro */}
            <path d="M100 0 Q100 60 600 110" stroke="hsl(var(--primary))" strokeWidth="2" strokeOpacity="0.3" fill="none" />
            <path d="M300 0 Q300 50 600 110" stroke="hsl(var(--primary))" strokeWidth="2" strokeOpacity="0.4" fill="none" />
            <path d="M500 0 Q500 40 600 110" stroke="hsl(var(--primary))" strokeWidth="2" strokeOpacity="0.5" fill="none" />
            <path d="M700 0 Q700 40 600 110" stroke="hsl(var(--primary))" strokeWidth="2" strokeOpacity="0.5" fill="none" />
            <path d="M900 0 Q900 50 600 110" stroke="hsl(var(--primary))" strokeWidth="2" strokeOpacity="0.4" fill="none" />
            <path d="M1100 0 Q1100 60 600 110" stroke="hsl(var(--primary))" strokeWidth="2" strokeOpacity="0.3" fill="none" />
            {/* Ponto central de convergência */}
            <circle cx="600" cy="110" r="6" fill="hsl(var(--primary))" opacity="0.8" />
          </svg>
        </div>
