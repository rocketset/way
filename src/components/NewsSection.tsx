...
const NewsSection = () => {
  ...
  if (isLoading) {
    return <section id="noticias" className="py-20 bg-gradient-to-b from-[#1a1a1a] to-[#0a0a0a]">
  ...
  return <section id="noticias" className="relative py-20 bg-gradient-to-b from-[#1a1a1a] to-[#0a0a0a] overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="mb-12 animate-fade-in text-center">
          <h2 className="text-4xl font-bold text-white mb-4">Conteúdos</h2>
          <p className="text-gray-300">
            Conteúdos exclusivos que estão virando o jogo no digital.
        ...
        loop: true
          ...
          <CarouselContent className="-ml-4">
            {blogPosts.map(post => <CarouselItem key={post.id} className="pl-4 md:basis-1/2 lg:basis-1/3">
                <Link to={`/blog/${post.slug}`} className="block group">
                  <div className="bg-card rounded-lg overflow-hidden border border-border hover:border-primary hover:shadow-lg hover:shadow-[0_0_30px_rgba(252,211,77,0.3)] transition-all duration-300 h-full">
                    <div className="relative h-64 overflow-hidden bg-gradient-to-br from-primary/20 to-primary/5">
                      {post.featured_image ? <img src={post.featured_image} alt={post.titulo} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" /> : <div className="w-full h-full flex items-center justify-center">
                    ...
                    <div className="p-6">
                      <div className="flex items-center gap-3 mb-3">
                        <span className="text-sm font-bold text-primary">
                          {format(new Date(post.criado_em), "dd MMM", {
                        locale: ptBR
                      }).toUpperCase()}
                        </span>
                        {post.categorias && post.categorias.length > 0 && <span className="px-3 py-1 bg-primary/10 text-primary text-xs font-semibold rounded-full">
