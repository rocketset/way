...
import { Heart, ArrowLeft, Save, Plus, Trash2, X } from "lucide-react";
...
import {
  useConductGuideContent,
  useUpdateConductGuideSection,
  useCreateConductGuideSection,
  useDeleteConductGuideSection,
} from "@/hooks/useConductGuideContent";
...
export default function ConductGuideEditor() {
  ...
  const createSection = useCreateConductGuideSection();
  const deleteSection = useDeleteConductGuideSection();
  ...
  const handleSave = () => {
    ...
    if (isCreating) {
      const maxOrdem = sections?.reduce((max, s) => Math.max(max, s.ordem), 0) || 0;
      createSection.mutate({
        section_key: formData.section_title.toLowerCase().replace(/\s+/g, '_'),
  ...
  const confirmDelete = () => {
    if (sectionToDelete) {
      deleteSection.mutate(sectionToDelete);
              ...
              <CardContent>
                <div className="space-y-6 mt-6">
                  <div className="space-y-2">
                    <Label htmlFor="new_section_title">Título da Seção</Label>
                    <Input
                  ...
                  <div className="flex gap-2">
                    <Button onClick={handleSave} disabled={createSection.isPending}>
